...
function toggleCat() {
  $(".categorie-plat").click(function () {
    ...
    $(this)
      .attr("name", "categoriePlatActive")
      .siblings()
      .attr("name", "categoriePlat");
  });
}
...
function addLiIngredient() {
  ...
  $.ajax({
    type: "POST",
    method: "POST",
    url: "index.php?action=addLiIngredient", // On va ajouter ces valeurs à la session, on les detruiras une fois la recette soumise
    data: {
    ...
    success: function (html) {
      ...
      if (nomIngredient == "" || Number.isNaN(quantite) || type == "") {
        ...
        html += dataS;
        html +=
          '<button type="button" name="remove" class="' +
          nbLi +
          ' remove"><i class="fas fa-minus"></i></button></li>';
        $("#ingList").append(html);
      }
    },
  });
...
$(document).on("click", ".remove", function () {
  ...
  $.ajax({
    type: "POST",
    method: "POST",
    url: "index.php?action=delLiIngredient",
    data: {
...
function addLiUstensile() {
  ...
  $.ajax({
    type: "POST",
    method: "POST",
    url: "index.php?action=addLiUstensile", // On va ajouter ces valeurs à la session, on les detruiras une fois la recette soumise
    ...
    success: function (html) {
      ...
      if (nomUstensile == "" || Number.isNaN(qteUstensile)) {
        ...
        html += dataS;
        html +=
          '<button type="button" name="remove2" class="' +
          nbLi +
          ' remove2"><i class="fas fa-minus"></i></button></li>';
        $("#ustList").append(html);
      }
    },
  });
...
$(document).on("click", ".remove2", function () {
  ...
  $.ajax({
    type: "POST",
    method: "POST",
    url: "index.php?action=delLiUstensile",
    data: {

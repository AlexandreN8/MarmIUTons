...
$(document).ready(function () {
  $("#addComment").on("click", function () {
    // On utilise l'event on click du bouton inserer
    var comment;
    ...
    if (comment.length != "") {
      // Condition d'acceptation du commentaire
      $.ajax({
        url: "index.php?action=commentaireInsert",
        method: "POST",
        dataType: "text",
        data: {
        ...
        success: function (response) {
          // Si la fonction reussis on ajoute le commentaire via client
          nbCom++; // On augmente le nbCom pour pouvoir aussi l'incrémenter dans le php
          $("#nbCom").text(nbCom + "Commentaires");
          $(".userComments").prepend(response);
          $("#mainComment").val("");
          window.location.replace(
            "index.php?action=detailsRecette&numRecette=" + nomR
          );
          // window.location.reload();
          // setInterval('location.reload()', 7000);
        },
      });
    } else alert("Votre commentaire doit pas être vide."); // popu condition non remplie
  });
...
function getAllComments(start, max) {
  ...
  $.ajax({
    url: "index.php?action=commentaireAll",
    method: "POST",
    dataType: "text",
    data: {
      getAllComments: 1,
